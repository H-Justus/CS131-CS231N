...
def mix_images(image1, image2, channel1, channel2):
    ...
    right = image2_ex[:, np.shape(image1_ex)[1]//2:, :]
...
def mix_quadrants(image):
    ...
    ### YOUR CODE HERE
    Top_left_ex = rgb_exclusion(image, 'R')
    Top_right_ex = dim_image(image)
    Bottom_left_ex = image**0.5
    Bottom_right_ex = rgb_exclusion(image, 'R')
    Top_left = Top_left_ex[:np.shape(Top_left_ex)[0] // 2, :np.shape(Top_left_ex)[1] // 2, :]
    Top_right = Top_right_ex[:np.shape(Top_right_ex)[0] // 2, np.shape(Top_right_ex)[1] // 2:, :]
    Top = np.concatenate((Top_left, Top_right), axis=1)
    Bottom_left = Bottom_left_ex[np.shape(Bottom_left_ex)[0] // 2:, :np.shape(Bottom_left_ex)[1] // 2, :]
    Bottom_right = Bottom_right_ex[np.shape(Bottom_right_ex)[0] // 2:, np.shape(Bottom_right_ex)[1] // 2:, :]
